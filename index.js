...
async function fetchUserTemplates() {
    ...
    try {
        const templatesRef = collection(window.db, "users", user.uid, "templates");
        const querySnapshot = await getDocs(query(templatesRef, orderBy("createdAt", "desc")));
        userTemplates = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    } catch (error) {
        console.error("Error fetching user templates:", error);
        showMessage('Could not load your saved templates.', 'error');
    }
}

async function initializeTemplateBrowser() {
    ...
    browseBtn.addEventListener('click', async () => {
        await fetchUserTemplates(); // Fetch latest templates each time modal is opened
        modal.classList.add('show');

        const staticCategories = [...new Set(emailTemplates.map(t => t.category))];
        let allCategories = ["My Templates", ...staticCategories];

        categoriesContainer.innerHTML = ''; // Clear old buttons
        ...
        populateTemplates("My Templates"); // Show user templates by default
    });
...
function populateTemplates(category) {
    ...
    const emailSubjectInput = document.getElementById('emailSubject'); // Assuming this exists
    const modal = document.getElementById('templateModal');
    ...
    if (category === "My Templates") {
        templatesToRender = userTemplates.map(t => ({
            title: t.name,
            description: t.subject,
            purposeText: t.body, // The body is the purpose
            subject: t.subject, // Store subject separately
            isUserTemplate: true
        }));
         if (templatesToRender.length === 0) {
            templatesContainer.innerHTML = `<p style="color: var(--text-medium); text-align: center;">You haven't saved any templates yet.</p>`;
            return;
        }
    } else {
        templatesToRender = emailTemplates.filter(t => t.category === category);
    }
    ...
    templatesToRender.forEach(template => {
        ...
        card.addEventListener('click', () => {
            if (template.isUserTemplate) {
                emailPurposeTextarea.value = template.purposeText; // The body
                // emailSubjectInput.value = template.subject; // Set the subject line
            } else {
                emailPurposeTextarea.value = template.purposeText;
                // emailSubjectInput.value = ''; // Clear subject for standard templates
            }
            emailPurposeTextarea.focus();
...
async function handleGenerateEmails() {
    ...
    try {
        ...
        if (!localProspects || localProspects.length === 0) {
            showMessage('No prospects to generate emails for.', 'error');
        ...
        const generationTask = {
            ...
            purpose: document.getElementById('emailPurpose').value.trim(),
            language: document.getElementById('emailLanguage').value,
            userId: window.auth.currentUser.uid // Assumes auth is exposed on window
        };
        ...
        console.error('Error preparing generation task:', error);
        showMessage('Failed to start email generation. Check inputs or try again.', 'error');
        showProcessing(false);
    }
}
